...
internal class RenderProcess : IDisposable
{
	...
	public CrossingwayRpc? Rpc { get; private set; }
	...
	{
		_keepAliveHandleName = $"CrossingwayRendererKeepAlive{pid}";
		_ipcChannelName = $"CrossingwayRendererIpcChannel{pid}";
		_dependencyManager = dependencyManager;
		_pluginDir = pluginDir;
		_configDir = configDir;
		_parentPid = pid;

		Rpc = new CrossingwayRpc(_ipcChannelName);
	...
	public void EnsureRenderProcessIsAlive()
	{
		...
		{
			Services.PluginLog.Error("Render process is crashing in a loop - please check the logs. No further restarts will be attempted until Crossingway is restarted.");
			Stop();
			Rpc?.Dispose();
			Rpc = null;
			OnProcessCrashed();
	...
	private Process SetupProcess()
	{
		...
		{
			FileName = Path.Combine(_pluginDir, "renderer", "Crossingway.Renderer.exe"),
