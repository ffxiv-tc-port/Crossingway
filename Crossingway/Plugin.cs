...
{
	private const string _command = "/cw";
	...
	public string Name => "Crossingway";
	...
	private void DependenciesReady()
	{
		...
		_renderProcess.Rpc!.RendererReady += msg =>
		{
			...
			{
				Services.PluginLog.Information("D3D11 shared textures not supported; using shared memory IPC.");
			}
		...
		_renderProcess.Rpc.UpdateTexture += msg =>
		{
			Services.Framework.RunOnFrameworkThread(() =>
			{
				...
				{
					if (msg.SharedMemoryName is string sharedMemName)
					{
						overlay.SetSharedMemoryTexture(sharedMemName, msg.Width, msg.Height);
					}
					else
					{
