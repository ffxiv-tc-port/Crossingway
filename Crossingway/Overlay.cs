...
internal class Overlay : IDisposable
{
	...
	{
		_captureCursor = cursor != Cursor.CrossingwayNoCapture;
	...
	public void Render()
	{
		...
		// TODO: Crossingway.Renderer can take some time to spin up properly, should add a loading state.
		...
		else
		{
			if (_texErrorIcon is not null)
			{
				...
				ImGui.Image(_texErrorIcon.GetWrapOrEmpty().ImGuiHandle, new Vector2(size, size));
	...
	private ImGuiWindowFlags GetWindowFlags()
	{
		...
		// Disable mouse inputs when overlay has no texture yet (prevents blank overlay from
		// stealing ImGui focus and consuming WndProc keyboard events from settings panel)
		if (_overlayConfig.ClickThrough || (!_captureCursor && locked) || _textureHandler == null)
		{
	...
	public void SetSharedMemoryTexture(string sharedMemName, int width, int height)
	{
		_resizing = false;
		_hasRenderError = false;
		...
		try
		{
			_textureHandler = new SharedTextureHandler(sharedMemName, width, height);
		}
		catch (Exception e) { _textureRenderException = e; }
