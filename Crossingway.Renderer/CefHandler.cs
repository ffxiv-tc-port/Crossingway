...
internal static class CefHandler
{
	...
	public static void Initialise(string cefAssemblyPath, string cefCacheDir, int parentPid)
	{
		...
		settings.UserAgentProduct = $"Chrome/{Cef.ChromiumVersion} Crossingway/{Assembly.GetEntryAssembly()?.GetName().Version} (ffxiv_pid {parentPid}; renderer_pid {Environment.ProcessId})";
