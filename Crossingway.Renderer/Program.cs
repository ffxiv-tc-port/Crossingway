...
internal static class Program
{
	...
	private static void Run(RenderParams args)
	{
		...
		_ = _rpc.RendererReady(dxRunning && DxHandler.HasSharedTextureSupport);
	...
	private static Task SendTextureUpdate(Guid guid, TextureRenderHandler renderHandler)
	{
		if (renderHandler.SharedMemoryName is string name)
			return _rpc.UpdateTextureSharedMemory(guid, name, renderHandler.SharedMemoryWidth, renderHandler.SharedMemoryHeight);
		return _rpc.UpdateTexture(guid, renderHandler.SharedTextureHandle);
	}
	...
	private static void RpcOnResizeOverlay(ResizeOverlayMessage msg)
	{
		lock (_lockIpc)
		{
			...
			if (_overlays.TryGetValue(guid, out var overlay))
			{
				...
				_ = SendTextureUpdate(guid, overlay.RenderHandler);
			}
		}
	}
	...
	private static void RpcOnNewOverlay(NewOverlayMessage msg)
	{
		lock (_lockIpc)
		{
			...
			var guid = new Guid(msg.Guid.Span);
			bool useSharedMemory = !DxHandler.HasSharedTextureSupport;
			var renderHandler = new TextureRenderHandler(size, guid, useSharedMemory);
